...
if [[ "$(which vagrant)" != "" && "$(which VBoxManage)" != "" && "$(which dkms)" != "" ]]; then
	echo "Passed."
else
...
if [[ "$(which dkms)" == "" ]]; then
	echo "DKMS is not installed, installing now..."
...
if [[ "$(which VBoxManage)" == "" ]]; then
	...
	SOURCE="deb http://download.virtualbox.org/virtualbox/debian $RELEASE contrib"
